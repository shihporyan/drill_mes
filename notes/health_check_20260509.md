...
| 動作 | commit | 結果（snapshot 對比 13:10 → 15:24） |
|---|---|---|
| 4 phantom 孔數歸零 | `38a8530` | `hole_count > 50M`: 4 → **0** ✓ |
| 2 純 April stubs 刪除 | 同上 | M08 4/3 + M13 4/20 deleted ✓（其他 3 筆有資料的保留） |
| 6 台 TZ 設定修正 | 同上 + deploy | machines.json 6 台 0→1 ✓；運行效果待新 LoadProgram 事件觸發驗證 |

### F2. Step B — laser parser bug 才是根因

第一次 backfill (15:15) 結果在下一個 live cycle (~15:18) 被 wipe。深挖發現 `parse_laser_machine` 對 `unique_wo` 中**所有**記錄 UPSERT，不論日期 — 每個 cycle 都用 today ± 1 day 的 events window 重算所有歷史 WO，超出 window 的 → hole_count = 0 → 覆蓋既有正確值。

| 角色 | 實證 |
|---|---|
| 倖存的 4/28-4/30 4 筆 | 操作員面板已從 ProcTimeEnd 清掉 → 不在 `unique_wo` → 不被 UPSERT 覆蓋 |
| 5/2-5/8 的 May 記錄 | 仍在 ProcTimeEnd → 每個 cycle 被覆蓋成 0 |

**Fix (`b246657`)：** UPSERT loop 加 `if start_dt.date() != target_date: continue`。

### F3. Step B 修復後 backfill v2 結果（15:50）

| 指標 | Before fix | After fix + backfill v2 |
|---|---|---|
| `laser_work_orders` nonzero rows | 4 | **14** (10 new) |
| `laser_work_orders` sum hole_count | 8,156 | **29,360** |
| L2 5/3 `TOP-WD-2604115-A` | 0 | **4,721** ✓ |
| 4/28-4/30 historical 4 筆 | 1044/3558/450/3104 | **保留不動** ✓ |

**新增的 10 筆 production WO hole_count：**

| 機 | 日期 | WO | holes |
|---|---|---|---|
| L2 | 5/3 | TOP-WD-2604115-A | 4,721 |
| L2 | 5/4 | BOT-WD2604125-A | 502 |
| L3 | 5/4 | BOT-WD2604125-C | 502 |
| L4 | 5/4 | BOT-WD2604125-B | 502 |
| L3 | 5/4 | WD-2604125-BOT-C | 502 |
| L4 | 5/4 | WD-2604125-B-1-0 | 3 |
| L2 | 5/5 | WD-2604125-BOT-A | 502 |
| L4 | 5/5 | WD-2604122-TOP-D | 2,493 |
| L3 | 5/6 | GR-2605003-A | 5,739 |
| L2 | 5/6 | GR-2605003-TOP-A | 5,738 |

### F4. 還沒做或無法做的事

| # | 議題 | 狀態 |
|---|---|---|
| 1 | L2 5/2 backfill | ❌ 不可做 — raw 在第一次 v1 backfill (15:15) 後被 cleanup retention (7d) 砍了。下次同類修復要趕在 raw 過期前 |
| 2 | TZ 6 台運行效果驗證 | ⏳ 需要 M03/M09/M10/M11/M16/M17 任一台再按 LOAD，下次 USB 採樣即可看到差距是否歸零 |
| 3 | 5/8 26% 是否假日 | ⏳ 需找排班員問 |
| 4 | Weekend toggle 預設值 | ⏳ UX 政策決定 |
| 5 | M14 FILE.Log MISSING | ⏳ 下次採樣前去 `\\<M14_ip>\LOG\09FILE.Log` 看 |

### F5. Commit timeline (5/9)

```
c61d273  Extend usb_sample with --audit window + SHA256 + schema/cycle_stats
3a7e827  usb_sample: live O100 SMB probe + machines.json
38a8530  Step A: phantom holes cleanup + 6 TZ fixes + audit report
232741e  probe_production_state for Step B planning
134eaeb  Step B: backfill_laser_holes (initial; later proven insufficient alone)
b246657  Fix parse_laser_machine wiping historical hole_count every cycle
```
