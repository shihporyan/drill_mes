...
def load_settings():
    ...
    Supports DRILL_DEV_CONFIG env var to override the config file path.
...
def get_machines_by_type(machines_config, machine_type):
    """Filter and return only enabled machines of a specific type.

    Args:
        machines_config: Full machines config dict.
        machine_type: Machine type string ('takeuchi' or 'kataoka').

    Returns:
        list: List of enabled machine dicts matching the given type.
    """
    return [
        m for m in machines_config["machines"]
        if m.get("enabled", False) and m.get("type") == machine_type
    ]


def get_backup_root(settings=None):
    """Resolve the backup root directory path.

    Args:
        settings: Optional settings dict. Loads from file if None.

    Returns:
        str: Absolute path to the backup root directory.
    """
    if settings is None:
        settings = load_settings()
    backup_root = settings.get("backup_root", "C:\\DrillLogs")
    if not os.path.isabs(backup_root):
        backup_root = os.path.join(PROJECT_ROOT, backup_root)
    return backup_root
